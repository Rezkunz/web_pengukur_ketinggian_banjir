...
// ─────────────────────────────────────────────
// SENSOR SIMULATOR LOGIC (FOR DEMONSTRATION)
// ─────────────────────────────────────────────
let simIntervalTimer = null;

function updateSimDisplay() {
    const range = document.getElementById('sim-water-range');
    const display = document.getElementById('sim-water-val-display');
    const badge = document.getElementById('sim-water-status-badge');
    if (!range || !display || !badge) return;

    const val = parseInt(range.value);
    display.textContent = `${val} cm`;

    // Ambil threshold saat ini
    const s1 = THRESHOLDS.SIAGA1;
    const s2 = THRESHOLDS.SIAGA2;

    if (val >= s2) {
        badge.textContent = 'SIAGA 2 (BAHAYA)';
        badge.style.background = 'rgba(239, 68, 68, 0.15)';
        badge.style.color = '#ef4444';
    } else if (val >= s1) {
        badge.textContent = 'SIAGA 1 (WASPADA)';
...
function handleSimulationMode() {
    const toggle = document.getElementById('toggle-sim-mode');
    const panel = document.getElementById('sim-control-panel');
    const range = document.getElementById('sim-water-range');
    if (!toggle || !panel || !range) return;

    // Sinkronkan range max dengan THRESHOLDS.MAX_TANK
    range.max = THRESHOLDS.MAX_TANK;

    if (toggle.checked) {
        panel.style.display = 'block';
        updateSimDisplay();

        // Mulai interval simulasi (kirim data ke Firebase tiap 2 detik)
        ...
        simIntervalTimer = setInterval(() => {
            ...
            // Tentukan status
            let statusStr = 'AMAN';
            if (val >= THRESHOLDS.SIAGA2) statusStr = 'SIAGA 2 (BAHAYA)';
            else if (val >= THRESHOLDS.SIAGA1) statusStr = 'SIAGA 1 (WASPADA)';

            // Kirim data sensor simulasi
            database.ref('sensor_data').update({
                water_level: val,
                status: statusStr,
                ts: firebase.database.ServerValue.TIMESTAMP
            }).catch(err => console.warn('Gagal kirim data simulasi:', err));
...
// Bind event listener untuk simulator
document.addEventListener('change', (e) => {
    if (e.target.id === 'toggle-sim-mode') {
        handleSimulationMode();
    }
});

document.addEventListener('input', (e) => {
    if (e.target.id === 'sim-water-range') {
        updateSimDisplay();
    }
});
